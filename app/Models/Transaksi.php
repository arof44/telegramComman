...
class Transaksi
{
    ...
    public function createTranskasiKeluar($request)
    {
        ...
        foreach ($request->id_barang as $key => $value) {
            ...
            if ($stokFix < 0) {
                $barang_gagal = DB::table('barang')->where('id', $value)->first();
                $sukses['valid'] = false;
                $sukses['barang'] = $barang_gagal->nama;
                DB::table('barang_stock')->where('id_transaksi', $data)->delete();
                DB::table('transaksi_item')->where('id_transaksi', $data)->delete();
                DB::table('transaksi')->where('id', $data)->delete();
                break;
            } else {
    ...
    public function sendNotifTelehabis($arr)
    {
        $pesan = [
            'text' => 'Hallo ' . Auth::user()->name . ' stok ' . $arr['nama_barang'] . ' sudah Habis !',
            'disable_notification' => true
        ];
        Auth::user()->notify(new TelegramNotification($pesan));
    }
